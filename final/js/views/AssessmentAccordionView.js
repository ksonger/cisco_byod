...
window.AssessmentAccordionView = Backbone.View.extend({
    ...
    updateMenu:function(val)   {
        var tgt_c = ($("#assessment_table").height()*.5);
        var tgt_q = ($("#assessment_table").height()*.5);
        for(var i=app.assessmentView.categories.length-1;i>-1;i--) {
            if(app.assessmentView.categories[i].$el.offset().top - $("#assessment_table").offset().top < tgt_c)   {
                var acc = this;
                ...
                for(var j=app.assessmentView.categories[i].questions.length-1;j>-1;j--)    {
                   if(app.assessmentView.categories[i].questions[j].$el.offset().top - $("#assessment_table").offset().top < tgt_q)   {
                        var tgt = app.assessmentView.categories[i].questions[j];
                        // maybe more than one that fit the criteria?
                        var alternate = null;
                        for(var k = j-1;k>-1;k--)   {
                            if(app.assessmentView.categories[i].questions[k].$el.offset().top - $("#assessment_table").offset().top < 0)    {
                                break;
                            }   else    {
    ...
    scrollMenu:function(elem)   {
        try{
            if((elem.offset().top) - $("#assessment_table").offset().top < 0) {
                //out of frame above the list
                var pos = ($("#assessment_nav").find("#list").getNiceScroll()[0].getScrollTop()) + (elem.offset().top - $("#assessment_table").offset().top);
                $("#assessment_nav").find("#list").getNiceScroll()[0].setScrollTop(pos);
            }
            if(elem.offset().top - $("#assessment_table").offset().top > $("#assessment_table").height()) {
                //out of frame below the list
                var h = (elem.parent().next().offset().top - elem.offset().top);
                var pos = h + ($("#assessment_nav").find("#list").getNiceScroll()[0].getScrollTop()) + ((elem.offset().top - $("#assessment_table").offset().top) - $("#assessment_table").height());
                $("#assessment_nav").find("#list").getNiceScroll()[0].setScrollTop(pos);
            }
        }   catch(e)    {
